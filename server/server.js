// BeatBear relay server — small rooms joined by code.
...
const PROTOCOL_VERSION = 5   // 4 → 5: 🐉 beam에 bang(굴절각)·blen(굴절 구간 길이)·bblk 추가 — 굴절을 소유자가 계산해 보냄(양쪽 동일)
...
function roster(code) {
  ...
  if (!room) return []
  return [...room.entries()].map(([id, p]) => ({ id, name: p.name, animal: p.animal, skin: p.skin, pattern: p.pattern, hat: p.hat, shape: p.shape, desk: p.desk, kb: p.kb, mouse: p.mouse }))
}
  ...
  ws.on('message', (raw) => {
    ...
    if (msg.t === 'join' && typeof msg.room === 'string' && !joinedRoom) {
      ...
      room.set(id, {
        ...
        desk: String(msg.desk || 'wood').slice(0, 12),
        kb: String(msg.kb || 'dark').slice(0, 12),
    ...
    if (msg.t === 'pos') {
      ...
      broadcast(joinedRoom, { t: 'beam', id, on: msg.on, nx: msg.nx, ny: msg.ny, ang: msg.ang, st: msg.st, len: msg.len, blk: msg.blk, bang: msg.bang, blen: msg.blen, bblk: msg.bblk }, id)   // 🐉 카메하메파 빔(len/blk=막힘, bang/blen/bblk=굴절 → 양쪽 동일 렌더)
    ...
    // ── 배틀/오버레이 연출·지형 릴레이(전부 상대 화면에도 동일하게 보이게) ──
    ...
    } else if (msg.t === 'gift-spawn') {
      broadcast(joinedRoom, { t: 'gift-spawn', id, gt: msg.gt | 0 }, id)          // 🎁 소유자 책상에 선물상자 등장(랜덤 종류 gt)
    } else if (msg.t === 'gift-claim') {
      ...
      if (typeof msg.desk === 'string') me.desk = msg.desk.slice(0, 12)
      if (typeof msg.kb === 'string') me.kb = msg.kb.slice(0, 12)
      if (typeof msg.mouse === 'string') me.mouse = msg.mouse.slice(0, 12)
...
console.log(`BeatBear relay listening on ws://0.0.0.0:${PORT}`)
