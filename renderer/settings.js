...
(function () {
  ...
  const inEdit = $('in-edit'), inServer = $('in-server'), inSafe = $('in-safe')
  const btnConnect = $('btn-connect'), btnDisconnect = $('btn-disconnect')
  const statusEl = $('status'), capacityEl = $('capacity')

  if (!api) { statusEl.textContent = '이 창은 비트베어 앱에서 열어야 합니다'; return }
  ...
  btnConnect.onclick = () => {
    ...
    if (!url) { statusEl.textContent = '서버 주소를 입력하세요'; statusEl.style.color = '#c9a2b0'; return }
    api.toOverlay({ t: 'connect', url })
  }
