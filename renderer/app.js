...
(function () {
  ...
  const SHAPE_KEYS = ['ear', 'eye', 'mouth', 'tail', 'hand', 'body']   // wire order for the packed shape string ('hand'/'body' 말단 추가 → 하위호환)
  function loadShape() {
  ...
  const isDev = !!(window.beatbear && window.beatbear.isDev)   // developer PC only (env BEATBEAR_DEV=1) → mint count currency freely
  ...
  // 외형 소유(꾸미기) — 기본 외형(APPEAR_DEFAULTS)만 처음부터 사용 가능, 나머지는 전부 외형 소환으로 획득. key = "group:value".
  // 개발자(isDev)는 전부 보유로 처리. Skins는 프리셋 id 단위 소유.
  const APPEAR_DEFAULTS = { skin: 'default', hat: 'none', ear: 'round', eye: 'round', mouth: 'smile', hand: 'round', kb: 'dark', mouse: 'white', desk: 'wood' }
  let ownedAppear = new Set()
  try { const a = JSON.parse(localStorage.getItem('ownedAppear') || '[]'); if (Array.isArray(a)) ownedAppear = new Set(a) } catch {}
  try { const h = JSON.parse(localStorage.getItem('ownedHats') || '[]'); if (Array.isArray(h)) h.forEach((v) => v !== 'none' && ownedAppear.add('hat:' + v)) } catch {}   // 구 ownedHats 마이그레이션
  let appearCounts = {}   // { "group:value": n } — 중복 스택(조합 재료)
  try { const c = JSON.parse(localStorage.getItem('appearCounts') || '{}'); if (c && typeof c === 'object') appearCounts = c } catch {}
  ownedAppear.forEach((k) => { if (!(appearCounts[k] > 0)) appearCounts[k] = 1 })   // 보유분 최소 1개 시드
  function saveAppear() { try { localStorage.setItem('ownedAppear', JSON.stringify([...ownedAppear])); localStorage.setItem('appearCounts', JSON.stringify(appearCounts)) } catch {} }
  function isAppearOwned(group, value) { return isDev || APPEAR_DEFAULTS[group] === value || ownedAppear.has(group + ':' + value) }
  function appearCount(group, value) { return appearCounts[group + ':' + value] || 0 }
  function grantAppear(group, value) { const k = group + ':' + value; const had = ownedAppear.has(k); ownedAppear.add(k); appearCounts[k] = (appearCounts[k] || 0) + 1; saveAppear(); return had }
  function isHatOwned(hat) { return hat === 'none' || ownedAppear.has('hat:' + hat) }
  // 소환 대상 풀 = 기본값을 제외한 모든 꾸미기 아이템(모든 하위 카테고리)
  function appearPool() {
    const A = window.AnimalArt; if (!A) return []
  ...
  // 조합(꾸미기): 아무 꾸미기 5개 소비 → 랜덤 꾸미기 1개. picks = [{group,value}×5]
  function craftAppear(picks) {
    if (!Array.isArray(picks) || picks.length !== 5) return { ok: false, reason: 'need5' }
    const need = {}; for (const p of picks) { const k = p.group + ':' + p.value; need[k] = (need[k] || 0) + 1 }
    for (const k in need) if ((appearCounts[k] || 0) - need[k] < 1) return { ok: false, reason: 'keep1' }   // 1개 초과만 재료(마지막 1개 보존)
    const pool = appearPool(); if (!pool.length) return { ok: false, reason: 'nopool' }
    for (const k in need) { appearCounts[k] = (appearCounts[k] || 0) - need[k]; if (appearCounts[k] <= 0) { appearCounts[k] = 0; ownedAppear.delete(k) } }
  ...
  let pawCoin = 0; try { pawCoin = Math.max(0, parseInt(localStorage.getItem('pawCoin') || '0', 10) || 0) } catch {}
  function addPawCoin(n) { pawCoin = Math.max(0, pawCoin + (n | 0)); try { localStorage.setItem('pawCoin', String(pawCoin)) } catch {} return pawCoin }

  // 🎁 선물 상자 — 기본 30분(타이핑 시 최대 10분)마다 책상 좌측에 랜덤 종류(0=Paw/1=Grizzle) 등장, 클릭 시 재화 1개(소유자 귀속).
  //  주기 = "유효시간" 누적: p += dt × speed, speed = clamp(1 + K×KPS, 1, MAXSPEED). MAXSPEED=3 → 하한 10분. 상자 떠 있으면 정지.
  const GIFT_BASE_MS = 15 * 60 * 1000   // 상자 등장 주기(고정 15분 — 타이핑 가속 없음)
  let giftTimerShown = false   // 하단바 클릭으로 남은시간 표기 토글(상시 아님)
  let giftProgress = 0; try { giftProgress = Math.max(0, Math.min(GIFT_BASE_MS, parseFloat(localStorage.getItem('giftProgress') || '0') || 0)) } catch {}
  if (isDev && giftProgress < GIFT_BASE_MS - 20000) giftProgress = GIFT_BASE_MS - 15000   // 🛠 개발자: 실행 후 ~15초 뒤 첫 상자(테스트 편의). 배포본은 정상 주기.
  let giftLastTick = performance.now(), giftSaveT = 0
  let myGift = null   // { type, born, state:'in'|'idle'|'open', openT }
  ...
  const me = {
    id: 'me',
    name: localStorage.getItem('name') || (window.beatbear && window.beatbear.osUser) || '나',   // 저장 이름 없으면 PC 계정 이름
    animal: 'cat',
    skin: localStorage.getItem('skin') || 'default',
    pattern: localStorage.getItem('pattern') || 'solid',
    hat: localStorage.getItem('hat') || 'none',
    deskStyle: localStorage.getItem('deskStyle') || 'wood',    // 꾸미기: 책상/키보드/마우스 외형
    kbStyle: localStorage.getItem('kbStyle') || 'dark',
    mouseStyle: localStorage.getItem('mouseStyle') || 'white',
  ...
  // 잠긴(미획득) 꾸미기는 기본값으로 되돌림(개발자는 전부 보유라 유지). 로드 시 + 조합 소비 후 호출. 소환으로만 활성화되게 하는 불변식.
  function enforceAppearOwnership(broadcast) {
    if (!isHatOwned(me.hat)) { me.hat = 'none'; localStorage.setItem('hat', 'none') }
    ;['ear', 'eye', 'mouth', 'hand'].forEach((g) => { if (me.shape && !isAppearOwned(g, me.shape[g])) me.shape[g] = APPEAR_DEFAULTS[g] })
    if (!isAppearOwned('desk', me.deskStyle)) me.deskStyle = 'wood'
    if (!isAppearOwned('kb', me.kbStyle)) me.kbStyle = 'dark'
    if (!isAppearOwned('mouse', me.mouseStyle)) me.mouseStyle = 'white'
    const A = window.AnimalArt, ps = (A && A.SKIN_PRESETS) || []
    const cur = ps.find((p) => p.tint === me.skin && p.body === (me.shape ? me.shape.body || 'plain' : 'plain'))
    if (cur && !isAppearOwned('skin', cur.id)) { me.skin = 'default'; me.tint = 'default'; if (me.shape) { me.shape.body = 'plain'; me.shape.tail = 'stub' } localStorage.setItem('skin', 'default') }
    localStorage.setItem('catShape', JSON.stringify(me.shape)); localStorage.setItem('deskStyle', me.deskStyle); localStorage.setItem('kbStyle', me.kbStyle); localStorage.setItem('mouseStyle', me.mouseStyle)
    if (broadcast) { try { sendUpdate(); pushState() } catch {} }
  }
  enforceAppearOwnership()
  ...
  // ---------- input source ----------
  let editing = false
  const inputSource = window.beatbear || {
    onInput(cb) {
  ...
  function modLabel(mod) {   // 없음/프리셋/커스텀 키 모두 처리
    ...
    if (mod === 'ctrlalt') return 'Ctrl+Alt'
    if (mod === 'ctrlshift') return 'Ctrl+Shift'
    if (mod === 'caps') return 'CapsLock'
    return mod   // 커스텀 조합키 이름(Space·F1·Ctrl 등) 그대로
  }
  function slotKeyLabel(i) { const k = String(keybinds.keys[i] || '?').toUpperCase(), m = modLabel(keybinds.mod); return m ? `${m}+${k}` : k }   // 조합키 없으면 슬롯 키만
  ...
  if (counterEl) counterEl.addEventListener('click', () => { giftTimerShown = !giftTimerShown })   // 🎁 하단바(카운터) 클릭 → 다음 상자 남은시간 표기 토글
  ...
  // 큰 수 축약: 100만↑ M, 1만↑ K
  function fmtCount(n) { n = Math.max(0, Math.floor(n || 0)); if (n >= 1e6) return (n / 1e6).toFixed(2).replace(/\.?0+$/, '') + 'M'; return n.toLocaleString() }   // K 단위 없음: 백만 미만은 전체 숫자, 백만↑만 M
  ...
  // Compares the last-seen version (localStorage) to the current app version and shows ONLY the most
  ...
  function showUpdateNotes() {
    ...
    unBody.innerHTML = ''
    for (const v of vers.slice(0, 1)) {   // 이번에 받은 "가장 최신" 노트 하나만 표시(사이 버전 노트는 생략)
  ...
  function drawPeerDimButtons(now) {
    ...
    const sc = view.scale, R = 11 * sc
    const BUB = window.AnimalArt.BUBBLE_H, DY = window.AnimalArt.DESK_Y
    for (let i = 0; i < catPos.length; i++) {
      const cat = allRef[i]; if (!cat || cat.id === 'me') continue   // opponents only
      const c = catPos[i]; if (!c) continue
      // 상대 책상의 왼쪽 끝 모서리 (catPos=머리중앙 기준 origin 역산)
      const originX = c.x - CELL_W / 2 * sc, originY = c.y - (BUB + 100) * sc
      const bx = originX + 15 * sc, by = originY + (BUB + DY + 26) * sc
      ...
      ctx.strokeStyle = dimmed ? 'rgba(120,180,255,0.9)' : 'rgba(200,205,220,0.5)'; ctx.lineWidth = 1.5 * sc; ctx.stroke()
      // 아이콘: 투명 활성=눈 감은(아래 호), 비활성=눈 뜬(아몬드+동공)
      ctx.strokeStyle = '#fff'; ctx.lineCap = 'round'; ctx.lineWidth = 1.6 * sc
      const er = R * 0.58
      if (dimmed) {
        ctx.beginPath(); ctx.arc(bx, by - er * 0.2, er, Math.PI * 0.12, Math.PI * 0.88); ctx.stroke()   // 감은 눈(속눈썹 호)
        for (const s of [-1, 0, 1]) { ctx.beginPath(); ctx.moveTo(bx + s * er * 0.6, by + er * 0.28); ctx.lineTo(bx + s * er * 0.7, by + er * 0.62); ctx.stroke() }
      } else {
        ctx.beginPath(); ctx.ellipse(bx, by, er, er * 0.64, 0, 0, Math.PI * 2); ctx.stroke()   // 뜬 눈
        ctx.fillStyle = '#fff'; ctx.beginPath(); ctx.arc(bx, by, er * 0.34, 0, Math.PI * 2); ctx.fill()
      }
      ctx.restore()
    }
  }
  ...
  function profileMsg() {
    return { name: me.name, animal: me.animal, skin: me.skin, pattern: me.pattern, hat: me.hat, shape: shapeStr(me.shape), desk: me.deskStyle, kb: me.kbStyle, mouse: me.mouseStyle }
  }
  ...
  function connect(url) {
    ...
    ws.onmessage = (ev) => {
      ...
      if (msg.t === 'joined') {
        ...
        if (myGift && myGift.state !== 'open' && net) net.send(JSON.stringify({ t: 'gift-spawn', gt: myGift.type }))   // 🎁 늦게 들어온 인원도 내 상자 보이게 재전송
        for (const id of [...peerGifts.keys()]) if (!msg.peers.some((p) => p.id === id)) peerGifts.delete(id)   // 나간 인원 상자 정리
        const seen = new Set()
        for (const p of msg.peers) {
          if (p.id === me.netId) continue
          seen.add(p.id)
          const ex = peers.get(p.id)
          if (ex) { ex.name = p.name; ex.tint = p.skin || 'default'; ex.pattern = p.pattern || 'solid'; ex.hat = p.hat || 'none'; ex.shape = parseShape(p.shape); ex.deskStyle = p.desk || 'wood'; ex.kbStyle = p.kb || 'dark'; ex.mouseStyle = p.mouse || 'white' }
          else { peers.set(p.id, { id: p.id, name: p.name, animal: 'cat', tint: p.skin || 'default', pattern: p.pattern || 'solid', hat: p.hat || 'none', shape: parseShape(p.shape), deskStyle: p.desk || 'wood', kbStyle: p.kb || 'dark', mouseStyle: p.mouse || 'white', feat: {}, ...newAnimState() }); setTimeout(resolveMyOverlap, 500) }   // 새 인원 등장 → 위치 도착 후 겹침 재확인
        }
        ...
        else {   // ac/ar 없이 정규화 좌표만 옴(구버전 or 드래그 중): 정지 상태면 가장 가까운 내 프리셋 칸으로 스냅(작업표시줄 정합=떠 보임 방지), 움직이는 중이면 원좌표(부드럽게)
          const rx = msg.nx || 0, ry = msg.ny || 0, W2 = canvas.clientWidth || 1, H2 = canvas.clientHeight || 1
          if (p._rawNx != null && Math.abs(rx - p._rawNx) < 0.005 && Math.abs(ry - p._rawNy) < 0.005) { const a = nearestAnchor(rx * W2, ry * H2, false); if (a) { p.nx = a.x / W2; p.ny = a.y / H2 } else { p.nx = rx; p.ny = ry } }
          else { p.nx = rx; p.ny = ry }
          p._rawNx = rx; p._rawNy = ry
        }
      ...
      else if (msg.t === 'error' && msg.reason === 'version_mismatch') { setStatus('버전 불일치 — 접속 불가'); showVersionBlockPopup(msg.sv, msg.cv); ws.close() }   // 서버-클라 버전 다름 → 접속 차단
    }
  ...
  if (window.BattleGachaUI && window.BattleGacha) {
    ...
    window.BattleGachaUI.setBridges({
      ...
      switchView: () => { try { window.beatbear.toOverlay({ t: 'next-monitor' }) } catch (e) {} }, // 🖥 화면 전환: 다음 모니터
    ...
    if (window.HGMenu) window.HGMenu.setBridges({
      ...
      setKeybinds: (kb) => { keybinds = { mod: kb.mod || 'alt', keys: (kb.keys || ['Z', 'X', 'C']).slice(0, 3) }; localStorage.setItem('keybinds', JSON.stringify(keybinds)); if (inputSource.setKeybinds) inputSource.setKeybinds(keybinds); refreshKeyUI(); pushState() },
      roomInfo: () => ({ connected: connected(), me: { name: me.name || '나', wins: battleWins, plays: battlePlays }, peers: [...peers.values()].map((p) => ({ id: p.id, name: p.name || ('#' + p.id), wins: p.bw || 0, plays: p.bp || 0 })) }),
      challenge: (id) => openBetDialog(id),
      weaponSlots: () => ({ keys: [0, 1, 2].map((i) => slotKeyLabel(i)), slots: me.slots.slice() }),
      setWeaponSlot: (i, id) => { me.slots[i] = id; localStorage.setItem('slots', JSON.stringify(me.slots)); if (battleActive) buildBattleHud(); pushState() },
      slotEligible: (id) => slotEligible(id),
      ...
      appearOwned: (group, value) => isAppearOwned(group, value),   // 모든 꾸미기 카테고리 소유 판정(개발자 전부 보유)
      appearCount: (group, value) => appearCount(group, value),     // 꾸미기 보유 개수(중복 스택)
      craftMats: (kind) => {   // 조합 재료 목록(보유 개수>0)
        if (kind === 'appear') return appearPool().filter((it) => appearCount(it.group, it.value) > 0).map((it) => ({ group: it.group, value: it.value, count: appearCount(it.group, it.value) }))
        const G = window.BattleGacha; if (!G || !G.catalog) return []
      ...
      setDeskStyle: (id) => { if (!window.AnimalArt.DESK_STYLES[id]) return; me.deskStyle = id; localStorage.setItem('deskStyle', id); sendUpdate() },
      setKbStyle: (id) => { if (!window.AnimalArt.KB_STYLES[id]) return; me.kbStyle = id; localStorage.setItem('kbStyle', id); sendUpdate() },
      setMouseStyle: (id) => { if (!window.AnimalArt.MOUSE_STYLES[id]) return; me.mouseStyle = id; localStorage.setItem('mouseStyle', id); sendUpdate() },
      // 소환(가챠) — 2재화: Paw Coin(외형 전용, app), Grizzle Coin(무기·소환체=BattleGacha gems). 개발자는 무제한.
      gachaCoins: () => ({ paw: isDev ? 999 : pawCoin, grizzle: isDev ? 999 : (window.BattleGacha ? window.BattleGacha.getGems() : 0) }),
      gachaPool: (kind) => {
        if (kind === 'appear') return appearPool().map((it) => ({ group: it.group, value: it.value, owned: isAppearOwned(it.group, it.value) }))
        const G = window.BattleGacha, D = window.BattleData; if (!G || !G.catalog || !D) return []
        return G.catalog().filter((e) => !e.starter && ((D.RARITY[e.rarity] || {}).weight || 0) > 0)
          .map((e) => ({ id: e.id, name: e.name || e.id, rarity: e.rarity, color: (e.rarityInfo || {}).color || '#c9a074', owned: e.owned }))
      },
      gachaOdds: () => {
        const D = window.BattleData; if (!D) return []
        const t = Object.values(D.RARITY).filter((r) => r.weight > 0), tot = t.reduce((s, r) => s + r.weight, 0)
        return t.map((r) => ({ key: r.key, name: r.name, color: r.color, pct: Math.round(r.weight / tot * 1000) / 10 }))
      },
      rollGacha: (kind, n) => {
        const G = window.BattleGacha; if (!G) return []
        const out = []
        for (let i = 0; i < (n || 1); i++) {
          if (kind === 'appear') {
            if (!isDev) { if (pawCoin < 1) break; addPawCoin(-1) }   // 🐾 Paw Coin 차감(개발자 무료)
            const pool = appearPool(); if (!pool.length) break
            const unowned = pool.filter((it) => !isAppearOwned(it.group, it.value))
            const src = unowned.length ? unowned : pool   // 미보유 우선, 전부 보유면 중복
            const pick = src[Math.floor(Math.random() * src.length)]
            const dup = grantAppear(pick.group, pick.value)   // 획득(신규면 dup=false)
            out.push({ group: pick.group, value: pick.value, appear: true, dup })
          } else {
            if (isDev && G.getGems() < 1) G.addGems(1)   // 개발자 무료(Grizzle)
            const r = G.roll(); if (!r) break
            out.push({ id: r.id, name: (r.entry && r.entry.name) || r.id, rarity: r.rarity.key, color: r.rarity.color, dup: r.dup, count: r.count })
          }
        }
  ...
  function pushState() {
    inputSource.pushState({
      name: me.name, skin: me.skin, pattern: me.pattern, hat: me.hat, shape: Object.assign({}, me.shape), deskStyle: me.deskStyle, kbStyle: me.kbStyle, mouseStyle: me.mouseStyle, slots: me.slots,
      ...
      ownedHats: [...ownedAppear].filter((k) => k.indexOf('hat:') === 0).map((k) => k.slice(4))
    })
  }
  ...
  inputSource.onCommand((msg) => {
    ...
    if (msg.t === 'request-state') pushState()
      ...
      if (msg.hat) { me.hat = (isDev || isHatOwned(msg.hat)) ? msg.hat : 'none'; localStorage.setItem('hat', me.hat) }   // reject locked hats(개발자는 전부 허용)
      // desk / keyboard / mouse cosmetics (꾸미기 하위 카테고리) — validate against the style lists
      if (msg.desk && window.AnimalArt.DESK_STYLES[msg.desk]) { me.deskStyle = msg.desk; localStorage.setItem('deskStyle', me.deskStyle) }
      if (msg.kb && window.AnimalArt.KB_STYLES[msg.kb]) { me.kbStyle = msg.kb; localStorage.setItem('kbStyle', me.kbStyle) }
      if (msg.mouse && window.AnimalArt.MOUSE_STYLES[msg.mouse]) { me.mouseStyle = msg.mouse; localStorage.setItem('mouseStyle', me.mouseStyle) }
      ...
      if (msg.keys && msg.keys.length) {
        ...
        refreshKeyUI()   // 단축키 바뀌면 배틀 HUD·메뉴·샵 라벨 최신화
        pushState()
      }
    }
  ...
  // 서버-클라 버전 불일치 팝업(접속 차단)
  function showVersionBlockPopup(sv, cv) {
    if (document.querySelector('.bm-verblock')) return
    const back = document.createElement('div'); back.className = 'no-drag bm-verblock'
    back.style.cssText = 'position:fixed;inset:0;z-index:2147483400;display:flex;align-items:center;justify-content:center;background:rgba(6,8,12,.55);font-family:system-ui,"맑은 고딕",sans-serif'
    const card = document.createElement('div')
    card.style.cssText = 'background:linear-gradient(180deg,#1a1f28,#12151b);border:1px solid #39414f;border-radius:14px;padding:22px 26px;width:min(360px,90vw);text-align:center;color:#e8ebf0;box-shadow:0 18px 50px rgba(0,0,0,.6)'
    card.innerHTML = `<div style="font-size:15px;font-weight:700;margin-bottom:10px">⚠️ 버전 불일치</div>` +
      `<div style="font-size:13px;color:#cfd4de;line-height:1.6;margin-bottom:6px">현재 접속하려는 서버의 버전과 맞지 않아<br>접속하실 수 없습니다.</div>` +
      `<div style="font-size:12px;color:#8fa0b4;margin-bottom:16px">서버 v${sv || '?'} · 내 앱 v${cv || '?'} — 앱·서버를 최신으로 맞춰주세요.</div>` +
      `<button class="verok" style="padding:9px 22px;border-radius:9px;border:1px solid #2f6bd8;background:#2f6bd8;color:#fff;font-weight:700;cursor:pointer">확인</button>`
    back.appendChild(card); document.body.appendChild(back); sendHotzone()
    card.querySelector('.verok').onclick = () => { back.remove(); sendHotzone() }
  }
  ...
  if (!window.beatbear) window.addEventListener('keydown', (e) => { if (e.key === 'F2') setEditing(!editing) })
  ...
  function startBattleSolo() {
    ...
    battleMulti = null
    _enterBattle()
    const dev = !!(window.beatbear && window.beatbear.isDev)
    ...
    for (let i = summonProj.length - 1; i >= 0; i--) {
      ...
      for (const [pid, rec] of remoteAnts) { if (now - rec.ts > 800) continue
        for (const e of rec.items.values()) { if (e.dead) continue; const key = pid + ':' + e.id; if (p.hitIds[key]) continue; const sp = remoteAntScreenPos(pid, e); if (!sp || Math.hypot(sp.x - p.x, sp.y - p.y) >= 24 * view.scale) continue
          ...
          const ehp = (e.hp || 1) + (e.sh || 0), pw = p.pow || p.dmg || 1   // 실드 포함 실효 HP
          if (connected()) net.send(JSON.stringify({ t: 'ant-hit', target: pid, ant: e.id, dmg: pw })); spawnSpark(p.x, p.y)
          if (pw >= ehp) e.dead = true                                           // 파워 ≥ HP = 죽임(로컬 즉시 표시 + 상대는 pw 데미지로 사망)
          if (pw > ehp) { p.pow = pw - ehp; penetrated = true }                  // 남은 파워 있으면 관통(계속 진행)
          else consumed = true                                                   // 딱 죽이거나(pw==ehp) 못 뚫으면(pw<ehp) 투사체 소멸
          break antloop }
      }
  ...
  setInterval(() => {
    ...
    if (ants.length) {
      ...
      { const nowA = performance.now(); net.send(JSON.stringify({ t: 'ants', list: ants.map((a) => ({ id: a.id, nx: +(a.x / NW).toFixed(4), hp: a.hp, sh: a.shHp || 0, dead: a.dead, dir: a.dir, sp: a.sprite, mhp: a.maxHp, sz: a.size, fz: (a.frozenUntil && a.frozenUntil > nowA) ? 1 : ((a.slowUntil && a.slowUntil > nowA) ? 2 : 0) })) })) }   // sh=실드(관통 판정용), fz: 1=빙결·2=감속
  ...
  function setDesktopMode(on) {
    ...
    try { window.beatbear && window.beatbear.setDesktopMode && window.beatbear.setDesktopMode(desktopMode) } catch {}
    showToast(desktopMode ? '🖥 바탕화면 모드 — 다른 창 뒤로(바탕화면에서만 보임)' : '🔝 오버레이 모드 — 항상 위')
  }
  // 시작 시 저장된 바탕화면 모드면 메인에 창 레이어 적용 요청(z-order만; 위치 변경 없음).
  if (desktopMode) { try { window.beatbear && window.beatbear.setDesktopMode && window.beatbear.setDesktopMode(true) } catch {} }
  ...
  // 접속 시 내 프리셋이 다른 피어와 겹치면 → 가장 가까운 빈 프리셋으로 이동(피어 위치가 도착한 뒤 호출).
  function resolveMyOverlap() {
    if (wx == null || !connected()) return
    const W = canvas.clientWidth, H = canvas.clientHeight
    let overlap = false
    for (const p of peers.values()) { if (p.nx == null || p.ny == null) continue; if (Math.abs(p.nx * W - wx) < cellPxW * 0.6 && Math.abs(p.ny * H - wy) < cellPxH * 0.6) { overlap = true; break } }
    if (!overlap) return
    const a = nearestAnchor(wx, wy, true)   // 가장 가까운 "빈" 프리셋(avoidPeers)
    if (a) { wx = a.x; wy = a.y; savedAnchor = { c: a.c, r: a.r }; localStorage.setItem('anchor', JSON.stringify(savedAnchor)); clampWidget(); positionHud(); sendHotzone() }
  }
  function initWidget() {
  ...
  function drawPeerCount(origin, taps) {
    ...
    const h = 32, pad = 4
    ctx.save()
    // warm bar (matches #hud-bar)
    ctx.beginPath(); ctx.roundRect(x, y, barW, h, 10)
    ctx.fillStyle = 'rgba(242,221,203,0.97)'; ctx.fill()
    ctx.lineWidth = 1; ctx.strokeStyle = '#e7b9a4'; ctx.stroke()
    // warm brown counter chip spanning the full inner width (no hamburger) — matches #counter
    ctx.beginPath(); ctx.roundRect(x + pad, y + pad, barW - pad * 2, h - pad * 2, 6)
    ctx.fillStyle = '#4a3324'; ctx.fill()
    ctx.fillStyle = '#ffe9c7'; ctx.font = '700 13px "Segoe UI", "Malgun Gothic", sans-serif'
  ...
  if (inputSource.onCursor) inputSource.onCursor((p) => {
    ...
    if (dragging) { wx = p.x - dragging.dx; wy = p.y - dragging.dy; clampWidget(); positionHud(); sendHotzone(); repositionMenu() }
  ...
  window.addEventListener('mouseup', () => {
    if (dragging) { dragging = null; snapToNearestAnchor(); repositionMenu() }   // drop → snap to nearest preset(+메뉴 재정렬)
  })
  ...
  // ---------- 🎁 선물 상자 ----------
  function tickGift(now) {
    const dt = Math.max(0, Math.min(1000, now - giftLastTick)); giftLastTick = now
    if (myGift && myGift.state === 'open' && now - myGift.openT > GIFT_OPEN_MS + 60) { myGift = null; giftProgress = 0; giftLastTick = now; saveGiftState() }   // 오픈 연출 끝 → 다음 쿨타임 새로 시작
    for (const [pid, g] of peerGifts) if (g.state === 'open' && now - g.openT > GIFT_OPEN_MS + 60) peerGifts.delete(pid)
    if (myGift && myGift.state === 'in' && now - myGift.born > 380) myGift.state = 'idle'
    for (const g of peerGifts.values()) if (g.state === 'in' && now - g.born > 380) g.state = 'idle'
    if (battleActive || myGift) return   // 배틀 중이거나 상자가 떠 있으면 주기 정지
    giftProgress += dt
    if (giftProgress >= GIFT_BASE_MS) spawnMyGift(now)
    if (now - giftSaveT > 4000) { giftSaveT = now; saveGiftState() }
  }
  function spawnMyGift(now) {
    myGift = { type: Math.random() < 0.5 ? 0 : 1, born: now, state: 'in' }; giftProgress = 0; saveGiftState()
    if (net && net.readyState === WebSocket.OPEN) net.send(JSON.stringify({ t: 'gift-spawn', gt: myGift.type }))
    try { showToast('🎁 책상에 선물 상자가 도착했어요') } catch {}
  }
  function claimGift() {   // 소유자(나)가 오픈 → 재화 획득 + 모두 오픈 연출
    if (!myGift || myGift.state === 'open') return
    myGift.state = 'open'; myGift.openT = performance.now(); awardGift(myGift.type); saveGiftState()
    if (net && net.readyState === WebSocket.OPEN) net.send(JSON.stringify({ t: 'gift-open' }))
  }
  function requestPeerGift(pid) { if (net && net.readyState === WebSocket.OPEN) net.send(JSON.stringify({ t: 'gift-claim', target: pid })) }   // 상대 상자 대신 클릭
  function giftRectFor(origin, scale) {   // 책상 좌측에 앉는 상자 스크린 좌표/반경
    const cx = origin.x + 30 * scale, cy = origin.y + (window.AnimalArt.BUBBLE_H + window.AnimalArt.DESK_Y - 8) * scale
  ...
  function giftPalette(type) {
    return type === 0
      ? { box: '#eab94c', boxD: '#cf9a34', lid: '#f4cd66', rib: '#fff3d6', ribD: '#e8cf9a', bow: '#fff3d6', em: '#8a5a28', coin: '#f2c236', coinE: '#b9862a', paw: '#6b4326', halo: '255,190,80' }
      : { box: '#d94f40', boxD: '#ab3729', lid: '#e56152', rib: '#ffcf6b', ribD: '#e0ab45', bow: '#ffcf6b', em: '#ffe0d6', coin: '#e2503f', coinE: '#a3271f', paw: '#7a2018', halo: '255,120,90' }
  }
  const giftEob = (u) => { const c1 = 1.70158, c3 = c1 + 1; return 1 + c3 * Math.pow(u - 1, 3) + c1 * Math.pow(u - 1, 2) }
  // 발바닥 주화 아이콘(화면좌표)
  function drawCoinIcon(ctx, X, Y, rad, type, alpha) {
    const P = giftPalette(type)
    ctx.save(); ctx.globalAlpha = alpha
    ctx.fillStyle = P.coin; ctx.strokeStyle = P.coinE; ctx.lineWidth = Math.max(1, rad * 0.14)
    ctx.beginPath(); ctx.arc(X, Y, rad, 0, Math.PI * 2); ctx.fill(); ctx.stroke()
    ctx.strokeStyle = 'rgba(255,255,255,0.35)'; ctx.lineWidth = Math.max(0.8, rad * 0.08); ctx.beginPath(); ctx.arc(X, Y, rad * 0.72, 0, Math.PI * 2); ctx.stroke()
    ctx.fillStyle = P.paw   // 발바닥(패드 + 발가락 4개)
    ctx.beginPath(); ctx.ellipse(X, Y + rad * 0.22, rad * 0.34, rad * 0.28, 0, 0, Math.PI * 2); ctx.fill()
    for (const dx of [-0.34, -0.11, 0.11, 0.34]) { ctx.beginPath(); ctx.ellipse(X + dx * rad, Y - rad * 0.3, rad * 0.11, rad * 0.15, 0, 0, Math.PI * 2); ctx.fill() }
    ctx.restore()
  }
  // 상자 본체(중심 cx,cy) — 뚜껑 들림/회전 파라미터
  function drawGiftBody(ctx, cx, cy, r, type, alpha, s, lidLift, lidRot, bodyRot) {
    const P = giftPalette(type), rrect = (x, y, w, h, rad) => { ctx.beginPath(); ctx.roundRect(x, y, w, h, rad) }
    ctx.save(); ctx.translate(cx, cy)
    ctx.globalAlpha = alpha * 0.55; ctx.fillStyle = 'rgba(40,30,25,0.3)'; ctx.beginPath(); ctx.ellipse(0, r * 0.86, r * 0.82, r * 0.2, 0, 0, Math.PI * 2); ctx.fill()
    ctx.globalAlpha = alpha; ctx.scale(s, s); if (bodyRot) ctx.rotate(bodyRot)
    const w = r * 1.6, h = r * 1.15, lh = r * 0.5, top = -h * 0.1
    ctx.fillStyle = P.box; rrect(-w / 2, top, w, h, 4); ctx.fill()
    ctx.fillStyle = P.boxD; rrect(w / 2 - r * 0.34, top, r * 0.34, h, 4); ctx.fill()
    ctx.fillStyle = P.rib; ctx.fillRect(-r * 0.17, top, r * 0.34, h)
    ctx.fillStyle = P.em
    if (type === 0) { const ey = top + h * 0.56; ctx.beginPath(); ctx.ellipse(-w * 0.26, ey, r * 0.3, r * 0.24, 0, 0, Math.PI * 2); ctx.fill(); for (const dx of [-0.14, 0.02, 0.18]) { ctx.beginPath(); ctx.ellipse(-w * 0.26 + dx * w, ey - r * 0.32, r * 0.09, r * 0.12, 0, 0, Math.PI * 2); ctx.fill() } }
    else { for (const dx of [-0.2, -0.02, 0.16]) { ctx.save(); ctx.translate(-w * 0.2 + dx * w, top + h * 0.5); ctx.rotate(0.3); ctx.beginPath(); ctx.roundRect(-r * 0.05, -r * 0.34, r * 0.1, r * 0.68, r * 0.05); ctx.fill(); ctx.restore() } }
    ctx.save(); ctx.translate(0, top - lidLift); ctx.rotate(lidRot)
    ctx.fillStyle = P.lid; rrect(-w * 0.56, -lh, w * 1.12, lh + 3, 4); ctx.fill()
    ctx.fillStyle = P.rib; ctx.fillRect(-r * 0.19, -lh, r * 0.38, lh + 3)
    ctx.fillStyle = P.bow; ctx.beginPath(); ctx.ellipse(-r * 0.42, -lh - r * 0.06, r * 0.36, r * 0.26, -0.5, 0, Math.PI * 2); ctx.fill(); ctx.beginPath(); ctx.ellipse(r * 0.42, -lh - r * 0.06, r * 0.36, r * 0.26, 0.5, 0, Math.PI * 2); ctx.fill()
    ctx.fillStyle = P.ribD; ctx.beginPath(); ctx.arc(0, -lh - r * 0.06, r * 0.15, 0, Math.PI * 2); ctx.fill()
    ctx.restore(); ctx.restore()
  }
  // target = 재화가 흡수될 캐릭터 지점(화면좌표)
  function drawGiftBox(ctx, g, cx, cy, r, now, target) {
    const P = giftPalette(g.type), cl = (v) => Math.max(0, Math.min(1, v))
    const tx = target ? target.x : cx, ty = target ? target.y : cy - r * 3
    if (g.state === 'open') {
      const u = cl((now - g.openT) / GIFT_OPEN_MS), et = now - g.openT
      const boxA = 1 - cl(u / 0.32) * 0.92
      if (boxA > 0.02) drawGiftBody(ctx, cx, cy, r, g.type, boxA, 1, cl(u / 0.24) * r * 1.6, cl(u / 0.24) * 0.5, 0)
      // 획득 재화 표시 네모칸(상자 위) — 잠깐 표시
      const cardCx = cx, cardCy = cy - r * 2.1
      const cardA = u < 0.5 ? cl((u - 0.05) / 0.14) : cl((0.6 - u) / 0.1)
      if (cardA > 0.02) { const cs = giftEob(cl((u - 0.05) / 0.2)) * r * 1.7; ctx.save(); ctx.globalAlpha = cardA; ctx.beginPath(); ctx.roundRect(cardCx - cs / 2, cardCy - cs / 2, cs, cs, 7); ctx.fillStyle = '#fff8ee'; ctx.fill(); ctx.strokeStyle = P.coin; ctx.lineWidth = 2.2; ctx.stroke(); ctx.restore() }
      // 코인: 공중에서 살짝 떠서 머물다(bob) → 캐릭터로 흡수(FLY_AT 이후)
      const FLY_AT = 0.66
      let coX = cardCx, coY = cardCy, coR = r * 0.55, coA = cl((u - 0.1) / 0.08)
      if (u <= FLY_AT) { coY = cardCy + Math.sin(et / 250) * r * 0.14 }   // 공중 머묾(둥실)
      else { const f = cl((u - FLY_AT) / (1 - FLY_AT - 0.04)), e = f < 0.5 ? 2 * f * f : 1 - Math.pow(-2 * f + 2, 2) / 2; coX = cardCx + (tx - cardCx) * e; coY = cardCy + (ty - cardCy) * e; coR = r * (0.55 - 0.36 * f); coA = 1 - cl((f - 0.72) / 0.28) }
      if (coA > 0.02) drawCoinIcon(ctx, coX, coY, coR, g.type, coA)
      // 흡수 스파클(캐릭터)
      if (u > 0.9) { const sp = cl((u - 0.9) / 0.1); ctx.save(); ctx.fillStyle = P.coin; for (let i = 0; i < 7; i++) { const a = i / 7 * Math.PI * 2, rad = r * sp * 1.5; ctx.globalAlpha = (1 - sp) * 0.9; ctx.beginPath(); ctx.arc(tx + Math.cos(a) * rad, ty + Math.sin(a) * rad, 1.7 * (1 - sp) + 0.6, 0, Math.PI * 2); ctx.fill() } ctx.restore() }
      return
    }
    // in / idle — 상자 + "떴다·눌러라" 주목 유도(할로 맥동 + 링 + 궤도 반짝, 텍스트/커서 없이)
    let s = 1, bob = 0, rot = 0
    if (g.state === 'in') s = giftEob(cl((now - g.born) / 380))
    else { bob = Math.sin(now / 460) * 2.0 * (r / 22); rot = Math.sin(now / 620) * 0.06 }
    const bcy = cy + bob, pulse = 0.5 + 0.5 * Math.sin(now / 500)
    const hr = r * (1.75 + 0.3 * pulse), hg = ctx.createRadialGradient(cx, bcy, r * 0.4, cx, bcy, hr)
    hg.addColorStop(0, 'rgba(' + P.halo + ',' + (0.34 * (0.55 + 0.45 * pulse)).toFixed(3) + ')'); hg.addColorStop(1, 'rgba(' + P.halo + ',0)')
    ctx.save(); ctx.fillStyle = hg; ctx.beginPath(); ctx.arc(cx, bcy, hr, 0, Math.PI * 2); ctx.fill(); ctx.restore()
    drawGiftBody(ctx, cx, bcy, r, g.type, 1, s, 0, 0, rot)
    ctx.save(); ctx.globalAlpha = (0.3 + 0.45 * pulse) * s; ctx.strokeStyle = P.coin; ctx.lineWidth = 2
    ctx.beginPath(); ctx.roundRect(cx - r * 1.08, bcy - r * 1.15, r * 2.16, r * 2.05, 8); ctx.stroke(); ctx.restore()
    for (let i = 0; i < 3; i++) { const a = now / 680 + i * 2.094, rad = r * 1.3, tw = cl(Math.sin(now / 250 + i * 2)); ctx.save(); ctx.globalAlpha = tw * 0.9 * s; ctx.fillStyle = '#fff6d0'; ctx.beginPath(); ctx.arc(cx + Math.cos(a) * rad, bcy + Math.sin(a) * rad * 0.7, 1.7 * tw + 0.5, 0, Math.PI * 2); ctx.fill(); ctx.restore() }
  }
  // 다음 상자까지 남은시간(현재 타이핑 속도 기준 실시간). 상자 떠 있으면 0.
  function giftRemainingMs() { return myGift ? 0 : GIFT_BASE_MS - giftProgress }
  function fmtMMSS(ms) { const s = Math.max(0, Math.ceil(ms / 1000)); return Math.floor(s / 60) + ':' + String(s % 60).padStart(2, '0') }
  function drawGiftTimer(ctx, cx, bottomY, now) {   // 캐릭터 위 말풍선(하단바 클릭 토글)
    const label = myGift ? '🎁 지금 열 수 있어요' : '🎁 ' + fmtMMSS(giftRemainingMs())
    ctx.save(); ctx.font = '700 13px "Malgun Gothic",sans-serif'; ctx.textBaseline = 'middle'
    const tw = ctx.measureText(label).width, w = tw + 18, h = 22, bx = cx - w / 2, by = bottomY - h - 5
    ctx.fillStyle = 'rgba(74,51,36,0.94)'; ctx.strokeStyle = '#ffe9c7'; ctx.lineWidth = 1.5
    ctx.beginPath(); ctx.roundRect(bx, by, w, h, 11); ctx.fill(); ctx.stroke()
    ctx.beginPath(); ctx.moveTo(cx - 4, by + h); ctx.lineTo(cx + 4, by + h); ctx.lineTo(cx, by + h + 5); ctx.closePath(); ctx.fill()
    ctx.fillStyle = '#ffe9c7'; ctx.textAlign = 'center'; ctx.fillText(label, cx, by + h / 2)
    ctx.restore()
  }
  ...
  function frame() {
    ...
    // 🎁 선물 상자(책상 위) — 내/상대 상자 렌더 + 클릭 히트 수집(배틀 중엔 숨김)
    giftHits = []
    if (!battleActive) all.forEach((p, i) => {
      const g = p.id === 'me' ? myGift : peerGifts.get(p.id)
      if (!g) return
      const rc = giftRectFor(origins[i], scale)
      giftHits.push({ owner: p.id, x: rc.x, y: rc.y, r: rc.r })
      const tgt = catPos[i] || { x: rc.x, y: rc.y - rc.r * 3 }   // 재화 흡수 지점 = 캐릭터
