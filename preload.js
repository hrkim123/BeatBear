...
contextBridge.exposeInMainWorld('beatbear', {
  ...
  // 🍔 햄버거 메뉴(별도 창) — 오버레이의 z-order·포커스를 건드리지 않는다
  menuOpen(payload) { ipcRenderer.send('menu-open', payload) },
  menuCloseReq() { ipcRenderer.send('menu-close-req') },
  menuMove(anchor) { ipcRenderer.send('menu-move', anchor) },
  menuSnap(snap) { ipcRenderer.send('menu-snap', snap) },
  onMenuAction(cb) { ipcRenderer.on('menu-action', (_e, m) => cb(m)) },
  onMenuInvoke(cb) { ipcRenderer.on('menu-invoke', (_e, m) => cb(m)) },
  menuInvokeResult(m) { ipcRenderer.send('menu-invoke-result', m) },
  onMenuClosed(cb) { ipcRenderer.on('menu-was-closed', () => cb()) },
