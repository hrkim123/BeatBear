# BeatBear 배포 · 자동 업데이트 가이드 (A안: electron-updater + GitHub Releases)
...
### 1. GitHub 저장소 만들기
1. https://github.com/new 에서 저장소 생성 (이름: `beatbear`, **Public 권장** — Private면 친구 앱도 토큰이 필요해 번거로움).
2. `package.json`의 `build.publish.owner`를 **네 GitHub 사용자명**으로 변경 (지금은 `YOUR_GITHUB_USERNAME`). `repo`는 `beatbear` 그대로.
   - (원하면 사용자명만 알려주면 내가 이 줄을 대신 수정해줄게.)
3. 로컬 저장소를 GitHub에 연결하고 올리기:
```powershell
git remote add origin https://github.com/<네사용자명>/beatbear.git
...
npm run dist
```
- 성공하면 `dist\BeatBear Setup 0.1.0.exe`가 생김(업로드는 안 함). 이 exe를 그냥 친구에게 보내 설치시켜도 됨(자동 업데이트만 없음).
...
### 5. 친구에게 전달 (처음 한 번만)
- Releases 탭의 **`BeatBear Setup x.x.x.exe`** 링크를 친구에게 전달 → 친구가 실행해서 설치.
...
자동 업데이트는 **클라이언트 앱**만 갱신합니다. 같이 놀려면 서버가 필요. **모델2 = 서버가 곧 방**(방 코드 없음, 한 서버에 접속한 사람끼리 함께 놀고 최대 12명):
- 한 명이 호스트: `npm run server`(개발 폴더) **또는** `beatbear-server.exe` 클릭(Node 불필요, 항상 최신 버전 서버).
- 같은 Wi-Fi면 나머지는 설정창 서버 주소에 `ws://<호스트IP>:8787` 입력(방 코드 없음).
- 인터넷으로 하려면 ngrok/Cloudflare Tunnel 또는 클라우드 VPS에 서버를 올리면 됨.
- **버전 게이트**: 클라·서버 버전 정확히 일치해야 접속 → **릴리스마다 서버도 최신으로**. exe 런처는 실행 시 최신 릴리스를 받아 뜨므로 자동 일치(재빌드 불필요).
- 서버 exe 빌드: `npm run server:exe`(→ `dist-server/beatbear-server.exe`). 릴리스에 함께 올려 친구가 클릭 실행하게 할 수 있음.
